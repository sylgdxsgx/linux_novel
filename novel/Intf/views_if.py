...
def add_event(request):
    ...
    if result:
        return JsonResponse({'status':10022,'message':'event id already exitsts'})

    result = Event.objects.filter(name=name)
    if result:
        return JsonResponse({'status':10023,'message':'event name already exitsts'})
...
def get_event_list(request):
    ...
    if name !='':
        ...
        if results:
            ...
            return JsonResponse({'status':10022,'message':'query request is empty'})
